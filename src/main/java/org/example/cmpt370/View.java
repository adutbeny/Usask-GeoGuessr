...
public class View extends StackPane implements Subscriber {
     ...
     * MUST call at the start of every window creation method */
    ...
    /** Displays with default background */
    /**
    ...
    public void selectMainMenu() {
        ...
        VBox buttonStack = new VBox(25);
        // If we have internet connection, display login buttons
        if (this.model.getInternetStatus()) {
            buttonStack.getChildren().addAll(this.quickplay, this.login, this.createAcc);
        } else {
            // No internet, display text message indicating this
            Text noInternet = new Text("Internet unavailable");
            noInternet.setFont(Font.font("Segoe UI This", 36));
    ...
    /** Loading screen for when a player is waiting for a match */
    ...
    /** Clears the text entry fields on the login in window
     * Does not also clear create account fields but that is not needed */
